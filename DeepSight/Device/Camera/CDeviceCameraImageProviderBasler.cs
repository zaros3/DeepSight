...
namespace HLDevice.Camera
{
	...
	public class CDeviceCameraImageProviderBasler : ICloneable
	{
        ...
        protected uint m_numberOfMultiGrabBuffersUsed = 4;
        protected uint m_numberOfMultiGrabStatus;
        protected bool m_bMultiShot = false;
        ...
        public object Clone()
        {
            CDeviceCameraImageProviderBasler objCamera = new CDeviceCameraImageProviderBasler();
            objCamera.m_converterOutputFormatIsColor = this.m_converterOutputFormatIsColor;
            objCamera.m_hConverter = m_hConverter;
            objCamera.m_convertedBuffers = m_convertedBuffers;
            objCamera.m_hDevice = m_hDevice;
            objCamera.m_hGrabber = m_hGrabber;
            objCamera.m_hRemovalCallback = m_hRemovalCallback;
            objCamera.m_hWait = m_hWait;
            objCamera.m_numberOfBuffersUsed = m_numberOfBuffersUsed;
            objCamera.m_numberOfMultiGrabBuffersUsed = m_numberOfMultiGrabBuffersUsed;
            objCamera.m_numberOfMultiGrabStatus = m_numberOfMultiGrabStatus;
            objCamera.m_bMultiShot = m_bMultiShot;
            objCamera.m_grabThreadRun = m_grabThreadRun;
            objCamera.m_open = m_open;
            objCamera.m_grabOnce = m_grabOnce;
            objCamera.m_grabThread = m_grabThread;
            objCamera.m_lockObject = m_lockObject;
            objCamera.m_buffers = m_buffers;
            objCamera.m_grabbedBuffers = m_grabbedBuffers;
            objCamera.m_callbackHandler = m_callbackHandler;
            objCamera.m_lastError = m_lastError;
            return objCamera;
        }
		...
		public CDeviceCameraImageProviderBasler()
		{
		...
		public void Open( uint index )
		{
            ...
            OpenDevice(Pylon.CreateDeviceByIndex(index));
		}
        ...
        public void MultiShotUseBuffers( uint iBufferCount )
        {
            m_numberOfMultiGrabBuffersUsed = iBufferCount;
        }
        ...
        public bool MultiShot()
        {
            ...
            {
                /* Set up the grabbing and start. */
                m_numberOfBuffersUsed = m_numberOfMultiGrabBuffersUsed;
                m_numberOfMultiGrabStatus = m_numberOfMultiGrabBuffersUsed;
                m_bMultiShot = true;
                m_grabOnce = false;
                m_grabThreadRun = true;
                m_grabThread = new Thread( Grab );
		...
		/* Open using device.*/
		protected void OpenDevice( PYLON_DEVICE_HANDLE device )
		{
			try {
				/* Use provided device. */
				m_hDevice = device;
                ...
                if ( false == Pylon.DeviceIsOpen(m_hDevice))
                {
		...
		protected void Grab()
		{
			...
			try {
				...
				while( m_grabThreadRun ) {
					// 대기 시간을 무한으로 늘린 경우, 카메라 그랩 이벤트가 들어오지 않으면 해당 스레드가 종료가 안되는 문제가 있음
					if( false == Pylon.WaitObjectWait( m_hWait, 1000 ) ) {
						continue;
					}
					...
					if( grabResult.Status == EPylonGrabStatus.Grabbed ) {
                        ...
                        /* Exit here for Multi Frame Mode */
                        if ( true == m_bMultiShot && 1 == m_numberOfMultiGrabStatus-- )
                        {
                            m_grabThreadRun = false;
                            break;
                        }
        ...
        public bool SetLineParameter( string strName, string strValue )
        {
            bool bReturn = false;
            do
            {
                if ( false == Pylon.DeviceFeatureIsAvailable( m_hDevice, "EnumEntry_" + strName + "_" + strValue ) )
                    break;
                Pylon.DeviceFeatureFromString( m_hDevice, strName, strValue );
